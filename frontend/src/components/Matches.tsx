...
const MATCHES = [
  {
    ...
    sign: 'Scorpio',
    hd: 'Generator',
    slots: ['Tue Eve ★', 'Thu Eve'],
    scheduleOverlap: 100,
    why: 'Both value depth and emotional presence. Date styles overlap — dinner + walks. Love language match: quality time.',
    status: 'confirmed',
    photos: [null, null],
  },
  {
    ...
    sign: 'Leo',
    hd: 'Projector',
    slots: ['Wed Aftn', 'Fri Eve ★'],
    scheduleOverlap: 80,
    why: 'Shared bucket list: concerts, galleries, day trips. Both open to wherever it goes. Strong lifestyle match.',
    status: 'pending',
    photos: [null, null],
  },
  {
    ...
    slots: ['Sat Morn ★'],
    scheduleOverlap: 60,
    why: 'Both serious about dating with intention. Receive love the same way — quality time, fully present.',
    status: 'pending',
    photos: [null, null],
  },
  {
    ...
    slots: ['Sun Eve ★'],
    scheduleOverlap: 90,
    why: 'Matching date energy — spontaneous, active. Bucket list overlaps on 6 items. Giving you a Friday night.',
    status: 'pending',
    photos: [null, null],
  },
  {
    ...
    sign: 'Aquarius',
    hd: 'Generator',
    slots: ['Fri Night ★', 'Sat Aftn ★'],
    scheduleOverlap: 70,
    why: 'Both want a real relationship. Physical warmth as a shared love language. Same music and food interests.',
    status: 'pending',
    photos: [null, null],
  },
];
    ...
    if (sel !== null) {
      ...
      return (
        <Screen className="matches-screen">
          <button onClick={() => this.setSel(null)} className="matches-back-btn">
            {copy.components.matches.matchCard.back}
          </button>
          <div className="fu d1 matches-card-header">
            <Mono className="matches-meta">
              {m.dist} · {m.sign} · {m.hd}
            </Mono>
          ...
          <div className="fu d3 matches-why-box">
            <Mono className="matches-why-label">
              {copy.components.matches.matchDetail.whyMatched}
            </Mono>
          ...
          <div className="fu d3 matches-schedule-box">
            ...
            <div className="matches-slots">
              {m.slots.map((s) => (
                <span
                  key={s}
                  className={`matches-slot-tag${s.includes('★') ? ' available' : ' unavailable'}`}
                >
                  {s}
                </span>
              ))}
          ...
          <div className="fu d4 matches-request-wrap">
            ...
            >
              {reqOpen
                ? copy.components.matches.matchDetail.cancelRequest
                : copy.components.matches.matchDetail.requestAvail}
            </button>
            {reqOpen && (
              <div className="fi matches-request-panel">
                ...
                <Btn
                  ...
                  onClick={() => {
                    this.setReqOpen(false);
                    this.setReqMsg('');
                  }}
                >
                  {copy.components.matches.matchDetail.sendRequest}
                </Btn>
              </div>
            )}
          </div>
          ...
          {m.status === 'confirmed' ? (
            <Mono className="matches-confirmed-message">
              {copy.components.matches.matchDetail.dateConfirmed}
            </Mono>
          ) : (
            <div className="matches-actions">
              <Btn onClick={() => this.setSel(null)}>
                {copy.components.matches.matchDetail.acceptMatch}
              </Btn>
              <Btn ghost onClick={() => this.setSel(null)}>
                {copy.components.matches.matchDetail.passMatch}
              </Btn>
            </div>
          )}
        </Screen>
      ...
      <Screen className="matches-screen">
        ...
        <div className="matches-list">
          {MATCHES.map((m, i) => (
            <div
              key={i}
              className="fu matches-list-anim"
              style={{ animationDelay: `${i * 0.07 + 0.08}s` }}
            >
              <div
              ...
              >
                <div>
                  <p className="matches-list-name">
                    {m.name}, {m.age}
                  </p>
                  <div className="matches-list-meta-row">
                    <Mono>{m.dist}</Mono>
                    {m.slots.some((s) => s.includes('★')) && (
                      <Mono className="matches-premium-slot">
                        {copy.components.matches.matchDetail.premiumSlot}
                      </Mono>
                    )}
                  </div>
                  {m.status === 'confirmed' && (
                    <span className="matches-list-status">
                      {copy.components.matches.matchCard.confirmed}
                    </span>
                  )}
                </div>
                {m.status !== 'confirmed' && (
                  <Mono className="matches-list-view-more">
                    {copy.components.matches.matchCard.viewMore}
                  </Mono>
                )}
              </div>
            </div>
