    ...
    return (
      <Screen>
        ...
        <div className="onboarding-main-container gap-28">
          <div className="fu d2">
            <Mono style={{ display: 'block', marginBottom: 12 }}>
              {copy.components.onboarding.obIdentity.iAmA}
            </Mono>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {copy.components.onboarding.obIdentity.genders.map((g) => (
                <Chip key={g} label={g} active={gender === g} onClick={() => this.setGender(g)} />
              ))}
            </div>
          </div>
          <div className="fu d3">
            <Mono style={{ display: 'block', marginBottom: 12 }}>
              {copy.components.onboarding.obIdentity.openTo}
            </Mono>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {copy.components.onboarding.obIdentity.seeking.map((s) => (
                <Chip
                  key={s}
                  label={s}
                  active={seeking.includes(s)}
                  onClick={() => this.toggleSeeking(s)}
                />
              ))}
            </div>
          </div>
          <div className="fu d4">
            <Mono style={{ display: 'block', marginBottom: 4 }}>
              {copy.components.onboarding.obIdentity.sunSign}
            </Mono>
            <p className="onboarding-p-hint-small">
              {copy.components.onboarding.obIdentity.sunSignHint}
            </p>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7 }}>
              {copy.components.onboarding.obIdentity.zodiacSigns.map((s) => (
                <Chip key={s} label={s} active={sign === s} onClick={() => this.setSign(s)} />
              ))}
            </div>
          </div>
        </div>
        <div className="fu d5" style={{ marginTop: 28 }}>
          <Btn
            onClick={() => {
              this.props.set({ gender, seeking, sign });
              this.props.go('ob-quiz');
            }}
            disabled={!gender || !seeking.length}
          >
            {copy.components.onboarding.obIdentity.continue}
          </Btn>
        </div>
      </Screen>
    );
