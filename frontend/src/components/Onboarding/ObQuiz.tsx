...
export const QS = [
  ...
  {
    id: 'time',
    label: copy.components.onboarding.obQuiz.q1.label,
  ...
  {
    id: 'intent',
    label: copy.components.onboarding.obQuiz.q2.label,
  ...
  {
    id: 'values',
    label: copy.components.onboarding.obQuiz.q3.label,
  ...
  {
    id: 'dates',
    label: copy.components.onboarding.obQuiz.q4.label,
  ...
  {
    id: 'love',
    label: copy.components.onboarding.obQuiz.q5.label,
  ...
  render() {
    ...
    return (
      <Screen key={step}>
        ...
        >
          <Mono>
            {step + 1} of {QS.length} — {q.label}
          </Mono>
          {step > 0 && (
            <button onClick={() => this.setStep(step - 1)} className="ob-quiz-btn-back">
              {copy.components.onboarding.obQuiz.back}
            </button>
          )}
        </div>
        ...
        {q.type === 'double' ? (
          <div className="fu d3" style={{ flex: 1 }}>
            <Mono style={{ display: 'block', marginBottom: 12 }}>
              {copy.components.onboarding.obQuiz.q5.giveLabel}
            </Mono>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 24 }}>
              {q.opts.map((o) => (
                <button
                  key={`g-${o}`}
                  onClick={() => this.setGive(o)}
                  className={`option-button quiz ${give === o ? 'selected' : ''}`}
                >
                  {o}
                </button>
              ))}
            ...
            <Mono style={{ display: 'block', marginBottom: 12 }}>
              {copy.components.onboarding.obQuiz.q5.receiveLabel}
            </Mono>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {q.opts.map((o) => (
                <button
                  key={`r-${o}`}
                  onClick={() => this.setReceive(o)}
                  className={`option-button quiz ${receive === o ? 'selected' : ''}`}
                >
                  {o}
                </button>
              ))}
            ...
            {q.opts.map((opt) => {
              ...
              return (
                <button
                  ...
                  className={`option-button quiz large ${on ? 'selected' : ''}`}
                >
                  {opt}
                </button>
              );
        ...
        <div style={{ marginTop: 20 }}>
          ...
          >
            {step < QS.length - 1
              ? copy.components.onboarding.obQuiz.next
              : copy.components.onboarding.obQuiz.continue}
          </Btn>
          {q.multi && (
            <p className="ob-quiz-p-select">{copy.components.onboarding.obQuiz.selectAll}</p>
          )}
        </div>
      </Screen>
    );
