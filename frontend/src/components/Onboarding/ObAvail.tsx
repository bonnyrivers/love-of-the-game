...
import { upsertOnboardingProfile } from '../../graphql/profileApi.ts';
import './ObAvail.css';
// ══════════════════════════════════════════════════════════════════════════════
// AVAILABILITY GRID
// ══════════════════════════════════════════════════════════════════════════════
    ...
    return (
      <Screen>
        <div className="fu d1" style={{ marginBottom: 22 }}>
          ...
          <p className="onboarding-p-subtitle">{copy.components.onboarding.obAvail.subtitle}</p>
        </div>
        <div className="fu d2" style={{ overflowX: 'auto', marginBottom: 20 }}>
          ...
          >
            <div />
            {DAYS.map((d) => (
              <div key={d} className="ob-avail-day-header">
                {d}
              </div>
            ))}
            {SLOTS.map((s) => (
              <>
                <div key={s} className="ob-avail-slot-label">
                  {s}
                </div>
                {DAYS.map((d) => {
                  const k = `${d}-${s}`,
                  ...
                  return (
                    <div
                      key={k}
                      onClick={() => this.toggle(d, s)}
                      className={`ob-avail-grid-cell ${on ? 'selected' : ''}`}
                    >
        ...
        <div
          className="fu d3"
          style={{ borderLeft: '1px solid var(--line)', paddingLeft: 12, marginBottom: 24 }}
        >
          <p className="ob-avail-p-notes">
            {copy.components.onboarding.obAvail.notes.map((note) => (
              <>
                {note}
                <br />
              </>
        ...
        <Btn onClick={this.handleComplete} disabled={count < 3}>
          {count < 3
            ? copy.components.onboarding.obAvail.pickMore
                .replace('{count}', 3 - count)
                .replace('{plural}', 3 - count === 1 ? '' : 's')
            : copy.components.onboarding.obAvail.allSet}
        </Btn>
      </Screen>
    );
  }
